...
#define NCMPOS 30
#define START 0
#define INTEGR CHARAC
#define KIMY DIMFLG
#define GARBFL DORES
#define INDEX INDEX1
#define ADDEND RESMO
#define POKER LINNUM
#define FDECPT VARPNT
#define LSTPNT FORPNT
#define ANDMSK FORPNT
#define EORMSK FORPNT+1
#define VARTXT OPPTR
#define DOMASK TANSGN
#define GRBPNT DEFPNT
#define SIZE JMPER+1
#define OLDOV JMPER+2
#define TEMPF3 DEFPNT
#define ARYPNT HIGHDS
#define GRBTOP LOWTR
#define DECCNT LOWDS
#define TENEXP LOWDS+1
#define DPTFLG LOWTR
#define EXPSGN LOWTR+1
#define DEGREE SGNFLG
#define DSCTMP FAC
#define INDICE FACMO
#define STRNG1 ARISGN
#define BUFPTR FBUFPT
#define STRNG2 FBUFPT
#define POLYPT FBUFPT
#define CURTOL FBUFPT
#define TXTPTR CHRGOT+1
#define ZSTORD STORDO
...
;
; --- GENERAL RAM ---:
CHARAC: .dsb 1 ;A DELIMITING CHARACTER.
;A ONE-BYTE INTEGER FROM "QINT".
#define INTEGR CHARAC
ENDCHR: .dsb 1 ;THE OTHER DELIMITING CHARACTER.
COUNT: .dsb 1 ;A GENERAL COUNTER.

; --- FLAGS ---:
DIMFLG: .dsb 1 ;IN GETTING A POINTER TO A VARIABLE
...
#define KIMY DIMFLG
VALTYP: .dsb 1 ;THE TYPE INDICATOR.
;0=NUMERIC 1=STRING.
#if INTPRC != 0

INTFLG: .dsb 1
#endif
DORES: .dsb 1 ;WHETHER CAN OR CAN'T CRUNCH RES'D WORDS.
;TURNED ON WHEN "DATA"
;BEING SCANNED BY CRUNCH SO UNQUOTED
;STRINGS WON'T BE CRUNCHED.
;WHETHER TO DO GARBAGE COLLECTION.
#define GARBFL DORES
SUBFLG: .dsb 1 ;FLAG WHETHER SUB'D VARIABLE ALLOWED.
...
INPFLG: .dsb 1 ;FLAGS WHETHER WE ARE DOING "INPUT"
;OR "READ".
TANSGN: .dsb 1 ;USED IN DETERMINING SIGN OF TANGENT.
#if REALIO != 0
...
TRMPOS: .dsb 1
#endif
LINWID: .byt LINLEN ;LENGTH OF LINE (WIDTH).
NCMWID: .byt NCMPOS ;POSITION BEYOND WHICH THERE ARE
;NO MORE FIELDS.
LINNUM: .byt 0 ;LOCATION TO STORE LINE NUMBER BEFORE BUF
;SO THAT "BLTUC" CAN STORE IT ALL AWAY AT ONCE.
.byt 44 ;A COMMA (PRELOAD OR FROM ROM)
...
TEMPPT: .dsb 1 ;POINTER AT FIRST FREE TEMP DESCRIPTOR.
;INITIALIZED TO POINT TO TEMPST.
LASTPT: .dsb 2 ;POINTER TO LAST-USED STRING TEMPORARY.
TEMPST: .dsb STRSIZ*NUMTMP ;STORAGE FOR NUMTMP TEMP DESCRIPTORS.
INDEX1: .dsb 2 ;INDEXES.
#define INDEX INDEX1
INDEX2: .dsb 2
RESHO: .dsb 1 ;RESULT OF MULTIPLIER AND DIVIDER.
#if ADDPRC != 0
...
#define ADDEND RESMO
.byt 0 ;OVERFLOW FOR RES.

; --- POINTERS INTO DYNAMIC DATA STRUCTURES ---;
TXTTAB: .dsb 2 ;POINTER TO BEGINNING OF TEXT.
;DOESN'T CHANGE AFTER BEING
;SETUP BY "INIT".
VARTAB: .dsb 2 ;POINTER TO START OF SIMPLE
;VARIABLE SPACE.
;UPDATED WHENEVER THE SIZE OF THE
;PROGRAM CHANGES, SET TO [TXTTAB]
;BY "SCRATCH" ("NEW").
ARYTAB: .dsb 2 ;POINTER TO BEGINNING OF ARRAY
;TABLE.
;INCREMENTED BY 6 WHENEVER
;A NEW SIMPLE VARIABLE IS FOUND, AND
;SET TO [VARTAB] BY "CLEARC".
STREND: .dsb 2 ;END OF STORAGE IN USE.
;INCREASED WHENEVER A NEW ARRAY
;OR SIMPLE VARIABLE IS ENCOUNTERED.
;SET TO [VARTAB] BY "CLEARC".
FRETOP: .dsb 2 ;TOP OF STRING FREE SPACE.
FRESPC: .dsb 2 ;POINTER TO NEW STRING.
MEMSIZ: .dsb 2 ;HIGHEST LOCATION IN MEMORY.

; --- LINE NUMBERS AND TEXTUAL POINTERS ---:
CURLIN: .dsb 2 ;CURRENT LINE #.
;SET TO 0,255 FOR DIRECT STATEMENTS.
OLDLIN: .dsb 2 ;OLD LINE NUMBER (SETUP BY ^C,"STOP"
;OR "END" IN A PROGRAM).
;SET UP LOCATION USED BY POKE.
#define POKER LINNUM
;TEMPORARY FOR INPUT AND READ CODE
OLDTXT: .dsb 2 ;OLD TEXT POINTER.
;POINTS AT STATEMENT TO BE EXEC'D NEXT.
DATLIN: .dsb 2 ;DATA LINE # -- REMEMBER FOR ERRORS.
DATPTR: .dsb 2 ;POINTER TO DATA. INITIALIZED TO POINT
;AT THE ZERO IN FRONT OF [TXTTAB]
;BY "RESTORE" WHICH IS CALLED BY "CLEARC".
;UPDATED BY EXECUTION OF A "READ".
INPPTR: .dsb 2 ;THIS REMEMBERS WHERE INPUT IS COMING FROM.

; --- STUFF USED IN EVALUATIONS ---:
VARNAM: .dsb 2 ;VARIABLE'S NAME IS STORED HERE.
VARPNT: .dsb 2 ;POINTER TO VARIABLE IN MEMORY.
;POINTER INTO POWER OF TENS OF "FOUT".
#define FDECPT VARPNT
FORPNT: .dsb 2 ;A VARIABLE'S POINTER FOR "FOR" LOOPS
;AND "LET" STATEMENTS.
;PNTR TO LIST STRING.
#define LSTPNT FORPNT
...
#define EORMSK FORPNT+1
OPPTR: .dsb 2 ;POINTER TO CURRENT OP'S ENTRY IN "OPTAB".
;POINTER INTO LIST OF VARIABLES.
#define VARTXT OPPTR
OPMASK: .dsb 1 ;MASK CREATED BY CURRENT OPERATOR.
;MASK IN USE BY RELATION OPERATIONS.
#define DOMASK TANSGN
DEFPNT: .dsb 2 ;POINTER USED IN FUNCTION DEFINITION.
;ANOTHER USED IN GARBAGE COLLECTION.
#define GRBPNT DEFPNT
DSCPNT: .dsb 2 ;POINTER TO A STRING DESCRIPTOR.
#if ADDPRC != 0
.dsb 1
#endif
FOUR6: .byt STRSIZ ;VARIABLE CONSTANT USED BY GARB COLLECT.
...
.byt 0
#endif
HIGHDS: .dsb 2 ;DESINATION OF HIGHEST ELEMENT IN BLT.
HIGHTR: .dsb 2 ;SOURCE OF HIGHEST ELEMENT TO MOVE.
TEMPF2:
#if ADDPRC != 0
.byt 0
#endif
LOWDS: .dsb 2 ;LOCATION OF LAST BYTE TRANSFERRED INTO.
LOWTR: .dsb 2 ;LAST THING TO MOVE IN BLT.
...
FAC:
FACEXP: .byt 0
FACHO: .byt 0 ;MOST SIGNIFICANT BYTE OF MANTISSA.
#if ADDPRC != 0

FACMOH: .byt 0
#endif
FACMO: .byt 0 ;MIDDLE ORDER OF MANTISSA.
FACLO: .byt 0 ;LEAST SIG BYTE OF MANTISSA.
FACSGN: .byt 0 ;SIGN OF FAC (0 OR -1) WHEN UNPACKED.
SGNFLG: .byt 0 ;SIGN OF FAC IS PRESERVED BERE BY "FIN".
...
#define INDICE FACMO
BITS: .byt 0 ;SOMETHING FOR "SHIFTR" TO USE.
...
ARISGN: .byt 0 ;A SIGN REFLECTING THE RESULT.
FACOV: .byt 0 ;OVERFLOW BYTE OF THE FAC.
;POINTER TO A STRING OR DESCRIPTOR.
#define STRNG1 ARISGN

FBUFPT: .dsb 2 ;POINTER INTO FBUFFR USED BY FOUT.
...
RNDX: .byt 128 ;LOADED OR FROM ROM.
...
* = 255 ;PAGE 1 STUFF COMING UP.
LOFBUF: .dsb 1 ;THE LOW FAC BUFFER. COPYABLE.
;---  PAGE ZERO/ONE BOUNDARY ---.
;MUST HAVE 13 CONTIGUOUS BYTES.
FBUFFR: .dsb (3*ADDPRC)+13 ;BUFFER FOR "FOUT".
...
.byt "P"
.byt "C"
.byt "("+128 ;MACRO DOESNT LIKE ('S IN ARGUMENTS.
#define Q 129
#define SPCTK 129
.byt 'T', 'H', 'E', 'N'|$80
#define THENTK 129
...
ERRTAB: .byt 'N', 'E', 'X', 'T', ' ', 'W', 'I', 'T', 'H', 'O', 'U', 'T', ' ', 'F', 'O', 'R'|$80
#define ERRNF 0
#define Q 16
.byt 'S', 'Y', 'N', 'T', 'A', 'X'|$80
#define ERRSN 16
#define Q 22
.byt 'R', 'E', 'T', 'U', 'R', 'N', ' ', 'W', 'I', 'T', 'H', 'O', 'U', 'T', ' ', 'G', 'O', 'S', 'U', 'B'|$80
#define ERRRG 22
#define Q 42
.byt 'O', 'U', 'T', ' ', 'O', 'F', ' ', 'D', 'A', 'T', 'A'|$80
#define ERROD 42
#define Q 53
.byt 'I', 'L', 'L', 'E', 'G', 'A', 'L', ' ', 'Q', 'U', 'A', 'N', 'T', 'I', 'T', 'Y'|$80
#define ERRFC 53
#define Q 69
.byt 'O', 'V', 'E', 'R', 'F', 'L', 'O', 'W'|$80
#define ERROV 69
#define Q 77
.byt 'O', 'U', 'T', ' ', 'O', 'F', ' ', 'M', 'E', 'M', 'O', 'R', 'Y'|$80
#define ERROM 77
#define Q 90
.byt 'U', 'N', 'D', 'E', 'F', 39, 'D', ' ', 'S', 'T', 'A', 'T', 'E', 'M', 'E', 'N', 'T'|$80
#define ERRUS 90
#define Q 107
.byt 'B', 'A', 'D', ' ', 'S', 'U', 'B', 'S', 'C', 'R', 'I', 'P', 'T'|$80
#define ERRBS 107
#define Q 120
.byt 'R', 'E', 'D', 'I', 'M', 39, 'D', ' ', 'A', 'R', 'R', 'A', 'Y'|$80
#define ERRDD 120
#define Q 133
.byt 'D', 'I', 'V', 'I', 'S', 'I', 'O', 'N', ' ', 'B', 'Y', ' ', 'Z', 'E', 'R', 'O'|$80
#define ERRDV0 133
#define Q 149
.byt 'I', 'L', 'L', 'E', 'G', 'A', 'L', ' ', 'D', 'I', 'R', 'E', 'C', 'T'|$80
#define ERRID 149
#define Q 163
.byt 'T', 'Y', 'P', 'E', ' ', 'M', 'I', 'S', 'M', 'A', 'T', 'C', 'H'|$80
#define ERRTM 163
#define Q 176
.byt 'S', 'T', 'R', 'I', 'N', 'G', ' ', 'T', 'O', 'O', ' ', 'L', 'O', 'N', 'G'|$80
#define ERRLS 176
#define Q 191
#if EXTIO != 0

.byt 'F', 'I', 'L', 'E', ' ', 'D', 'A', 'T', 'A'|$80
#define ERRBD 191
#define Q 200
#endif
.byt 'F', 'O', 'R', 'M', 'U', 'L', 'A', ' ', 'T', 'O', 'O', ' ', 'C', 'O', 'M', 'P', 'L', 'E', 'X'|$80
#define ERRST 200
#define Q 219
.byt 'C', 'A', 'N', 39, 'T', ' ', 'C', 'O', 'N', 'T', 'I', 'N', 'U', 'E'|$80
#define ERRCN 219
#define Q 233
.byt 'U', 'N', 'D', 'E', 'F', 39, 'D', ' ', 'F', 'U', 'N', 'C', 'T', 'I', 'O', 'N'|$80
#define ERRUF 233
#endif
...
DEC TXTPTR+1
#endif
LDA #((BUF&255)-1)&255 ;MAKE TXTPTR POINT TO
...
EXIGNT: .asc "?EXTRA IGNORED"

.byt 13
.byt 10
.byt 0
TRYAGN: .asc "?REDO FROM START"
...
.byt 12
.asc "WRITTEN "
.asc "BY WEILAND & GATES"

.byt 13
.byt 10
.byt 0
#endif
MEMORY: .asc "MEMORY SIZE"
.byt 0
TTYWID:
#if KIMROM == 0
...
#endif
#if REALIO == 0
.asc "SIMULATED BASIC FOR THE 6502 V1.1"
#endif
#if REALIO-1 == 0
.asc "KIM BASIC V1.1"
#endif
#if REALIO-2 == 0
.asc "OSI 6502 BASIC VERSION 1.1"
#endif
#if REALIO-3 == 0
.asc "### COMMODORE BASIC ###"
.byt 13
